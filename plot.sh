...
# plot "output" ls 12 lw 0.5 pt 1 ps 0.5 t ''

# set xrange [-20:20]

set format y "%.2f"
set offsets 0.01, 0.01, 0.01, 0.00

t(x) = exp (- (x-3) * (x-3) / (2*5*5)) / sqrt(2*pi*5*5)
binwidth=0.8
bin(x,w)=w*floor(x/w)

plot "output" u (bin($1,binwidth)):(1.0/(binwidth*`wc -l < output`)) smooth frequency with boxes ls 10 lw 1 t '', \
     t(x) ls 12 t ''
